...
## 2026-07-16 - Hacker engine and specialist lesson

- Locked the fourth hero as a 6-HP, Move-3 Hacker whose reusable cardinal-range Jam reduces the next exact enemy activation by 2 damage and whose one-charge Blackout replaces it with a stationary zero-damage HOLD. Both actions allow prior movement, ignore interception, consume the Hacker action, and are fingerprinted into the exact plan.
- Authored optional Chapter 4, System Override: Jam rewrites a Rugger's D1-to-D2 strike from 3 damage to 1, then Blackout disables a Lane Sentinel so Sniper can fire through its former grid. The two-turn simulation leaves the three core onboarding chapters and mission unlock threshold unchanged.
- Added deterministic coverage for Hacker stats, cardinal targeting, combatant-transparent LOS, obstacle/Data Block blockers, zero-damage attack rejection, action/charge economy, stale plans, byte-equivalent Jam plans, Drainer healing, Whale slam reduction/cancellation, immediate Sentinel bypass and exact HOLD resolution. The focused engine suite passes all 63 cases.

## 2026-07-16 - Hacker production set and complete specialist slice

- Completed optional Chapter 4, System Override, without changing the three-chapter core onboarding gate. `trainingCompleted >= 3` remains core-complete; value 4 records the optional Hacker specialist certification.
- Shipped the 6-HP, Move-3 Hacker as a training-lab operator with reusable cardinal-range Jam and one-charge Blackout. Jam reduces the next exact activation damage by 2, minimum 0, without changing route or target; Blackout replaces that exact activation with stationary zero-damage `HOLD` and disables interception while pending.
- Generated one approved 166x166 SpriteCook master (`c0608002-9691-4b1b-b6fe-ad812cbc48df`) and one grouped animation run (`c373c196-3c3f-4ca9-9fa2-2405fda93e55`). The single batch produced six native-180x180-frame sheets: idle, walk, Jam, Blackout, hurt, and death.
- The complete Hacker generation pass cost 114 credits and left a balance of 454. No standalone per-animation generation loop was used.
- Expanded the shared battle preload registry to 28 sheets: 24 across Guardian, Sniper, Pusher, and Hacker plus four Lane Sentinel sheets.
- Completed System Override through semantic browser interactions at 1440px and 1024px, inspected the designed phone battle notice, and captured zero console errors. Jam's 3-to-1 Rugger strike, Blackout's exact Sentinel `HOLD`, disabled grid, Sniper follow-up, and terminal specialist completion matched serialized state.
